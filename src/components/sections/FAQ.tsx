        ...
        <Accordion className="w-full space-y-4">
          {faqData.map((item) => (
            <AccordionItem
              key={item.id}
              value={item.id}
              className="bg-white/[0.02] border border-white/5 rounded-xl px-6 py-1 hover:border-white/10 transition-colors duration-200"
            >
              <AccordionTrigger className="text-sm sm:text-base font-semibold text-white hover:text-indigo-300 hover:no-underline text-left py-4 cursor-pointer">
                {item.question}
              </AccordionTrigger>
